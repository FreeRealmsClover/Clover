...
| Service | Purpose |
|---|---|
| `sanctuary.mysql` | MariaDB database |
| `sanctuary.webapi` | HTTP API used by the website/launcher (login, registration, portraits) |
| `sanctuary.gateway` | World/zone server the game client connects to after login |
| `sanctuary.login` | Login/auth server |
...
- A copy of `.env` with the required secrets (see below)
...
3. From `src/Docker`:
   ...
   docker compose up -d
   ```
4. Point a FreeRealms client and launcher at your server's login port.
...
- Never expose MariaDB's port publicly - it's internal-only in this compose file by design.
- Set real, unique values for every secret in `.env`.
...
Built on top of the open source [Sanctuary](https://github.com/Open-Source-Free-Realms/Sanctuary) FreeRealms server emulator by Open Source Free Realms, licensed under AGPL-3.0. This repository is Clover's own actively developed codebase, not a passive mirror - see [LICENSE](LICENSE) for full attribution.
